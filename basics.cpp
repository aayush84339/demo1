...
int grid(char arr[5][5], int x, int y){
    if (x==4 && y==3 || x==3 && y==4){return 1;}
    int s1=0,s2 = 0;
    if (arr[x][y+1]=='O' && y+1<5){s1 = grid(arr, x, y+1);}
    if (arr[x+1][y]=='O' && x+1<5){s2 = grid(arr, x+1, y);}
    return s1+s2;
}
...
int main(){
    ...
    char arr[5][5] = {{'O','O','O','O','O'},{'O','X','O','X','O'},{'O','O','O','O','O'},{'X','O','X','O','O'},{'O','O','O','X','O'}};
    cout << grid(arr, 0, 0);
